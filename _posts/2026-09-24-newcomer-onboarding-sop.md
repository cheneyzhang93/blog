...
| 往后每周 | 写 10 分钟 | 固定一天发进展：本周、风险、需要支持、下周 | 一次次攒起来的同步记录 | 到那天它真发出去了，里面有一条是要对方拍板的 |
...
怎么算做到位：这件事成了习惯——每次同步里都有一条要对方拍板的事，不用别人来问。这些同步攒下来就是一本账——你做了什么、提前报过什么风险、要过什么支持、对方怎么回的，全在。等到评价那天，不用争辩，把记录拿出来就行：吵起来往往是因为两边各记各的账，有记录在，账只有一套。
...
入职不是等着被安排，是件得自己张罗起来的事：把期待谈成约定，把过程变成记录，把评价落回事实。这个系列的另一端，是坐在桌子另一边的人该做的动作——22 个当场要用的判断，收在[《带人场景手册》]({{ '/posts/people-management-playbook/' | relative_url }})里。两篇的索引都收在[《职场治理》系列目录]({{ '/categories/职场治理/' | relative_url }})。
